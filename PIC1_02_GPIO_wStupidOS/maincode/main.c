...
// ===================== CONFIG OPTIONS =====================
#define AUTOSAR_SPIRIT_ENABLE     (1)  /* 1 = enable Autosar Spirit modules, 0 = disable */
...
#if AUTOSAR_SPIRIT_ENABLE
#include "Autosar_spirit/Port/Port.h"
#include "Autosar_spirit/IoHwAb/IoHwAb.h"
#endif
...
void __interrupt() ISR(void)
{
    if (PIR1bits.TMR2IF) {
        PIR1bits.TMR2IF = 0;
        /* Direct register write, not IoHwAb_ToggleLed(IOHWAB_LED_ALIVE):
         * PIC16F877A's 8-level hardware call stack is shared between
         * mainline and ISR. Routing this through IoHwAb->Dio pushed the
         * linker's worst-case call-depth estimate to 9 and tripped XC8
         * warning 1393 ("possible hardware stack overflow"). Keeping the
         * ISR on a flat register write removes its contribution to that
         * shared depth entirely -- see IoHwAb.h for the full writeup. */
        PORTA ^= DEVKIT_LED_02_ACT;
        OS_TimerISR();
    }
...
void task_1ms(void)
{
    ...
    IoHwAb_ToggleLed(IOHWAB_LED_HEARTBEAT);
}
...
void task_100ms(void)
{
    ...
    IoHwAb_ToggleLed(IOHWAB_LED_ERROR);
}